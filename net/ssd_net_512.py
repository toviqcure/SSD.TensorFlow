...
class VGG16Backbone(object):
    ...
    def ssd_conv_block(self, filters, strides, name, padding='same', reuse=None, kernel_size=3):
        with tf.variable_scope(name):
            conv_blocks = []
            conv_blocks.append(
            ...
            conv_blocks.append(
                    tf.layers.Conv2D(filters=filters * 2, kernel_size=kernel_size, strides=strides, padding=padding,
                        data_format=self._data_format, activation=tf.nn.relu, use_bias=True,
                        kernel_initializer=self._conv_initializer(),
                        bias_initializer=tf.zeros_initializer(),
